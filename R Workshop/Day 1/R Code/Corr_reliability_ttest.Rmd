...
favstats(~A2, data = bfi)

tally(~gender, data = bfi)


  summary(mutate(bfi, gender = as.factor(gender)))
...
bfi$A1.r2 <- min(bfi$A1, na.rm = TRUE) + max(bfi$A1, na.rm = TRUE) - bfi$A1
```
...
bfi$agreeable2 <- rowMeans(select(bfi, A1.r, A2, A3, A4, A5), na.rm = TRUE)
```

Let's take a look at it with a visualization. Also try faceting by gender.

```{r}
qplot(y = agreeable2, x = as.factor(gender), data = bfi, bins = 10, geom = "boxplot") +
  coord_flip()
```
...
```{r}
t.test(bfi$agreeable, mu = 3.5, data = bfi)

t.test(bfi$agreeable ~ bfi$gender, data = bfi)
```
...
x2 <- chisq.test(counts)
names(x2)

x2$expected

tally(education~gender, data = filter(bfi, !is.na(education)), format = "percent")
```
