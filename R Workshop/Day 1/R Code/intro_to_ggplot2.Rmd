...
```{r}
qplot(data = acitelli) +
  geom_histogram(aes(x = satisfaction, color = gender), bins = 10)
```
...
```{r}
qplot(x = satisfaction, fill = Gender, data = acitelli, alpha = .05, geom = "density") + 
...
```{r}
qplot(y = satisfaction, x = wise_hus, fill = wise_hus, data = menOnly, geom = "boxplot")
```
...
```{r}
ggplot(acitelli, aes(x = satisfaction)) +
  geom_histogram(aes(fill = Gender), bins = 12, alpha = 0.5)
```
...
ggplot(acitelli, aes(x = tension, y = satisfaction)) +
  geom_jitter() +
  geom_smooth(aes(color = Gender), method = "lm", se = 0)
```
