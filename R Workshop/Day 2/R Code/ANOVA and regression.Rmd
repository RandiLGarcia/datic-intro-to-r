...
?bfi
####t-test
t.test(conscient ~ gender, data = bfi)
####side-by-side boxplot of conscientiousness by gender
ggplot(bfi, aes(x = as.factor(gender), y = conscient)) + 
  geom_boxplot()

boxplot(conscient ~ gender, data = bfi)

qplot(x = as.factor(gender), y = conscient, data = bfi, geom = "boxplot") 
...
# ggplot(bfi, aes(x = as.factor(factor(education, labels=c('HS', 'finished HS',
#                                                 'some college',
#                                                 'college graduate',
#                                                 'graduate degree'))), 
#                 y = conscient)) +
...
favstats(conscient ~ education, data = bfi)

round(tally(~education, data = bfi, format = "proportion"), 2)
```
...
#summary(aov(conscient ~ education, data = bfi))
...
bfi <- bfi %>%
  tidyr::unite(gen_edu, gender, education, remove = FALSE) %>%
  dplyr::mutate(gender = as.factor(factor(gender, labels=c('Men', 'Women'))),
         gen_edu = as.factor(gen_edu))

mosaic::tally(~gen_edu, data = bfi)
mosaic::tally(gender ~ education, data = bfi)
...
favstats(conscient ~ gen_edu, data = bfi)
```
...
mod2.1 <- aov(conscient ~ education + gender, data = bfi)

summary(mod2.1)


mod2.2 <- aov(conscient ~ education:gender, data = bfi)
...
ggplot(plotdata2, aes(x = education, y = mean, fill = gender)) +
...
t.test(SATV, SATQ, data = sat.act_men, paired = TRUE)
```
...
```{r}
mod3 <- aov(score ~ topic*gender + Error(id/gender), 
            data = sat.act_long)
